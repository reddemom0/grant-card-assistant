/**
 * Analyze Dropbox folder with smart filtering and path exclusion
 * Filters out: old files, videos, images, huge files, and specified paths
 */

import { listDropboxFolder } from '../src/tools/dropbox.js';

const FOLDER_PATH = process.argv[2] || '/Granted Team Folder/Sales';
const EXCLUDE_PATH = process.argv[3] || '/Granted Team Folder/Sales/Grants';
const MIN_YEAR = 2023; // Only files from 2023 onward
...
async function analyzeWithFilters(folderPath, excludePath) {
  console.log('\n' + '='.repeat(80));
  console.log('📊 SMART DROPBOX ANALYSIS (WITH FILTERS & EXCLUSIONS)');
  console.log('='.repeat(80));
  console.log(`\n📂 Folder: ${folderPath}`);
  console.log(`🚫 Excluding: ${excludePath}`);
  ...
  try {
    ...
    let files = result.files; // Already filtered to files only by listDropboxFolder

    // Filter out excluded path
    const filesBeforeExclusion = files.length;
    // Use includes() to match paths containing the excluded subfolder
    files = files.filter(file => !file.path.includes('/Grants/'));
    const excludedCount = filesBeforeExclusion - files.length;

    console.log(`✅ Found ${filesBeforeExclusion} total files`);
    console.log(`🚫 Excluded ${excludedCount} files from ${excludePath}`);
    console.log(`📊 Analyzing ${files.length} remaining files\n`);
    ...
    // Display results
    console.log('='.repeat(80));
    console.log('✅ INDEXABLE FILES (2023+, <50MB, good format)');
    console.log('='.repeat(80));
    ...
    if (stats.wrongFormat.length > 0) {
      ...
      Object.entries(byExt).sort((a, b) => b[1] - a[1]).slice(0, 10).forEach(([ext, count]) => {
        console.log(`      .${ext}: ${count} files`);
      });
      if (Object.keys(byExt).length > 10) {
        console.log(`      ... and ${Object.keys(byExt).length - 10} more extensions`);
      }
    }
    ...
    console.log(`railway run node scripts/index-dropbox-rag.js --path "${folderPath}" --exclude "${excludePath}" --year ${MIN_YEAR}\n`);
...
analyzeWithFilters(FOLDER_PATH, EXCLUDE_PATH);
