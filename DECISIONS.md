...
## 2026-09-18 — Cards: answer first, name the slip, and find the subject in a DM

**Three fixes from using the cards.**

**1. /watch answered too slowly, and Chat said "Oracle is not responding".** Starting a watch read the post, searched our grants table, listed possible clients and sent the joining DM — three systems — before anything appeared. Now `startWatch` does nothing but parse and acknowledge; `setUpWatch` does all of it in the background and posts the finished card. "Watch this" answers the press and sends the joining DM afterwards (`sendJoiningDm`). Nothing provisional is posted, so there is still exactly one card per program in the space. Tests hold the grants table and HubSpot open and assert the turn finishes and the press is answered first.

**2. @mentioning Oracle and nobody else repeated the question.** "find 30 min with @Oracle" answered "Who should be on the call?", as if the mention had not been seen. `splitMentions` (people.js) now says what it dropped, and `normalizeChatEvent` counts app mentions — in a space the first is how Oracle was addressed, so a second is Oracle named as a person; in a DM, where no mention is needed, the first already is. /meet, /track ("pass to @Oracle", "remove @Oracle") and the review card now answer **"That's my own account — @mention the people you want on the call"** (with wording per card). The listener account counts the same way.

**3. In a DM, a card could not see the message it was meant to act on.** Chat gives each DM message its own thread, so the trigger *is* the thread's first message and there was nothing above it. `src/cards/subject.js` (`findSubject`, with `readRecent` in track-thread.js) reads the last five messages of that conversation and takes the nearest one that looks like what the card needs — the program post for /watch, the ask for /track, the review request and its Doc links for /review, the contact details for lead triage. **If more than one could be it, or the only candidate is a guess, the card quotes what it picked and asks** rather than acting. Outside a DM a fallback message must also be recent (30 minutes), because a space holds many conversations at once and the message before a command is often about something else. Lead triage also recognises being asked outright ("triage this lead"), which is how the request reads in a DM, where the details are in the message before.

**Impact:** `src/cards/subject.js` (new), `src/cards/track-thread.js` (`readRecent`), `src/cards/people.js` (`splitMentions`), `src/api/chat-google.js` (`appMentions`), `src/cards/{watch-card,meet-card,track-card,lead-card,lead-parse,review-card,commands}.js`, and DM tests for all four cards in `tests/unit/{watch-card,meet-card,track-card,lead-card,intro-card,lead-parse,review-card}.test.js`. No migration, no new scope, no console step.
